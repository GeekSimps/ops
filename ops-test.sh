...
5. sudo lxc-create -n pulse-prime -t download -- --dist alpine --release 3.20 --arch amd64
   sudo lxc-start -n pulse-prime
   sudo lxc-ls --fancy
   sudo lxc-attach -n pulse-prime
	

6. sudo lxc-attach -n pulse-prime -- ip addr add 10.0.0.2/24 dev eth0
   ...
   qemu-system-x86_64 \
    ...
    sudo ip addr add 10.0.0.3/24 dev br-horizon
    sudo ip link set br-horizon up
